...
import {
  ...
  ErreurDto,
  HeroDto,
} from './super-heroes.api.spec.dto';
...
export abstract class SuperHeroesApiSpecControllerBase {
  ...
  _getAllSuperHeroes(): Promise<ArrayOfHeroesDto[]> {
    return this.getAllSuperHeroes();
  }

  abstract getAllSuperHeroes(): Promise<ArrayOfHeroesDto[]>;
  ...
  @ApiResponse({ status: 500, type: ErreurDto })
  _getSuperHeroInformations(@Param('heroId') heroId: string): Promise<HeroDto> {
    return this.getSuperHeroInformations(heroId);
  }

  abstract getSuperHeroInformations(heroId: string): Promise<HeroDto>;
}
